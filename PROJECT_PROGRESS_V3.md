...
1. **FIX-2** dynamic document-entity discovery (same discovery layer as fundamentals; pack reference `utils/hkex_announcement_pdf.py` for PDF text extraction) — closes the INTERMEDIARIES + WHITEWASH_CONCERT gate FAILs. Optional hardening while there: async job for document-entities if parsing gets heavy.
2. Then re-run the 0.4 gate on a fresh unseen code — target 18/18 with the middle gap still DEFERRED. Heavy-issuer fundamentals/announcements now go through the async jobs (`/admin/fundamentals/job`, `/admin/announcements/job`).
3. Non-blocking backlog: fundamentals deep-parser refinement (insurer labels, wrong-scale guards); corporate-timeline 5y async job; portal surfaces for job triggers.
4. Production DB now: disclosure_interests 00388=108 / 01810=230 / 02318=628; fundamentals 02318=4 + 00941=4 periods; announcements 02318=236 + 00941=196; raw_provenance 202.
