...
5. **NEXT (§8 ROI order):** enrichment (beneficial_owner/placing_agent/adviser from DT data when Joe's gap data lands — lands within days); 02318 share-capital persistence run; corporate-timeline 5y async job; main↔authority branch reconciliation (P0 done, now permissible); API key rotation (64-zero key); RTSS Layer-1 compatibility confirmed (RTSS reads Turso directly — see round-4 digest §7).
