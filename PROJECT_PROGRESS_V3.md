...
- [~] **0.4** Unseen-stock gate RUN 2026-09-18 on company machine, stock = **02318 (PING AN INSURANCE)** — `PREEXISTING_ROWS_BY_DOMAIN` measured first: all zeros across stocks/ccass_snapshots/ccass_holdings/announcements/disclosure_interests/fundamentals/document_entities (candidates 02318/00941/01299/02020 all zero; 02318 chosen). Per-domain verdicts (all triggered from stock code alone, production Render):
...
- [ ] **0.5** Scoped fixes from the 0.4 run, in order: **FIX-1 dynamic fundamentals — CODE LANDED + production-partially-verified 2026-09-19** (commits `36859ff`+`1b4c485`+`88a8357`): whitelist `HKEX_FUNDAMENTAL_DOCUMENTS` DELETED; discovery walks the stock's announcement stream (2y window, rowRange 400), title-classifies reporting documents (results announcement > full report > overseas-regulatory republication), year-sanity period derivation, per-period ranked backup candidates (cover-only PDFs degraded loudly via LABELS_NOT_MATCHED), attempt budget 6 + 45s run budget + 140-page/25MB caps. **Production: 02318 = ready, 4 periods, real figures (rev H1-2026 = 171,879) — the 0.4 FAIL is functionally closed for moderate issuers; 00388/02020/00941 local E2E green (00388 matches whitelist-era values).** **REMAINING (merge into FIX-3):** heavy issuers (00941 China Mobile) still crash the sync route in production (single 100+-page parse exceeds the whole request window on free-tier CPU even standalone) — fundamentals must move to the async-job pattern (DI template); known limits to record per row: insurer statement phrasing only partially covered (02318 fields 2-3), occasional wrong-scale captures possible (label-parser brittleness — every value carries source_document provenance for audit). **FIX-2** dynamic document-entity discovery feeding `DOCUMENT_ENTITY_SPECS` (same discovery layer; pack reference `utils/hkex_announcement_pdf.py`). **FIX-3** async-job pattern for announcements/timeline/fundamentals + DI full-window chunked runner. Then re-run this same 0.4 gate until 18/18 (middle gap stays DEFERRED by design). **Pack-verified answers for all three fixes are stored at `docs_reference_evidence/latest_reference_updates/2026-09-18_FRIEND_PACK_ANSWERS_FOR_FIX123.md`** (FIX-2 = full working reference in the pack; FIX-1 = discovery half proven, parser half ours to build; FIX-3 = friend caps at 2y, we chunk+async+accumulate).
...
**Next session's task (scoped, P0 gate):**
1. **FIX-1 residual + FIX-3 (merge):** convert fundamentals to the async-job pattern (reuse `/admin/disclosure-interests/job` infra) so heavy issuers (00941-class) stop crashing the sync route; add server-side chunked windows. Then **FIX-2** dynamic document-entity discovery (same discovery layer, pack reference `utils/hkex_announcement_pdf.py`). Full scope in §7 0.5.
2. Then re-run the 0.4 gate on a fresh unseen code — target 18/18 with the middle gap still DEFERRED.
3. DI is production-proven (00388/01810/02318 = 966 rows total in Turso); do not regress it while fixing the above.
4. Production DB now: disclosure_interests 00388=108 / 01810=230 / 02318=628; 02318 has announcements 236, holdings 357, stock row 1; raw_provenance 202.
5. Local tooling on the company machine (workspace root, outside repo): `friend_pack/` (extracted authoritative pack), `turso_query.py` (Turso HTTP helper), `gate_04.py` (gate runner), `debug_dion.py` (DION flow reproducer with dumps).
5. **Round-4 notes digested 2026-09-19** → `docs_reference_evidence/latest_reference_updates/2026-09-19_FRIEND_ROUND4_DIGEST_RTSS_TAPE_WARM_TURSO.md`. Key takeaways: (a) friend's warm cache is shallow (summaries, history-date unconfirmed) — our Turso/archive warm path already serves dated full history; (b) **Longbridge tape is today-only — any tape/爆量 history must be captured daily or it is lost forever (Phase-1 candidate, Evidence-Cache pattern); 爆量訊號已證實冇超額報酬（616 樣本 p=0.600）— future screener must be labelled 報時+累積, not buy signal**; (c) **RTSS 搜股系統 = separate future project** (own stack: longport+libsql+streamlit, private repo) — reads from our data layer, never built inside this platform (§2 governance); (d) **CCASS middle-gap data = Joe's Drive folder; DT = DisclosureTracker 財技網站; gap data ETA ~days (2026-09-19)** — build the labelled import path when it lands; DT's six 財技事件 types (配股/供股/全購/合股/拆股/CB) map 1:1 onto our Phase-1 event_type; (e) `15092026 原webbsite係點拎DATA.md` re-uploaded 2026-09-19 (was empty) — four-role data-flow map: Webb-site source → local one-time extraction → Turso → Render API OR direct-Turso read; **rule adopted: sibling systems (RTSS/monitors) read Turso directly, the Render API serves humans/external queries**.
