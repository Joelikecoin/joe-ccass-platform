...
1. **0.5 fixes** (see §7 0.5): FIX-1 dynamic fundamentals + FIX-2 dynamic document-entity discovery (kills both whitelist root causes), FIX-3 announcements/timeline/DI async+chunked pattern. Then re-run the 0.4 gate on 02318 (it is now a known stock, so pick a fresh unseen code for the re-run per the gate's spirit) — target 18/18 with the middle gap still DEFERRED.
