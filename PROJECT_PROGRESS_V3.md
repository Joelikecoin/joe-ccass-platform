...
1. **0.4 unseen-stock gate** — pick a genuinely unseen stock (exclude `00388 01810 00004 00006 00362 00372 08226 01168 01211`), measure `PREEXISTING_ROWS_BY_DOMAIN` first (Turso baseline: `disclosure_interests` now has 338 rows — 00388=108, 01810=230; fundamentals/document_entities still 0), then trigger all domains from stock code alone and verify persistence → reload → restart/redeploy → second query, no manual intervention.
2. If a non-gap domain fails, fix the earliest failing dependency (§7 0.5) and re-run the same gate. DI is proven, so the gate now exercises the remaining domains end to end.
3. DI pagination nuance (non-blocking): DION's list counter can exceed rows whose event date falls inside the window (`DI_DATE_WINDOW_DIFFERENCE` warning) — acceptable as labelled; revisit only if a gate demands exact counter parity.
