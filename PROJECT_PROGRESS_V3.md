...
1. **Re-run the 0.4 gate on a fresh unseen code** — target 18/18 with the middle gap still DEFERRED. All FIX-1/2/3 fixes are live: heavy-issuer fundamentals/announcements via `/admin/fundamentals/job` + `/admin/announcements/job`; document-entities + light-issuer fundamentals via sync routes. Honest labelled zeros (COMPLETE_ZERO_RECORDS / NO_..._DISCOVERED) count as PASS-with-label, never FAIL.
2. Non-blocking backlog: fundamentals deep-parser refinement (insurer labels, wrong-scale guards); corporate-timeline 5y async job; portal surfaces for job triggers.
3. Production DB now: disclosure_interests 00388=108 / 01810=230 / 02318=628; fundamentals 02318=4 + 00941=4 periods; announcements 02318=236 + 00941=196; document_entities 02318=9 rows (3 types); raw_provenance 202.
