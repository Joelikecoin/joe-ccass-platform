...
8. **REMAINING (honest, gap excluded):** ① enrichment columns (beneficial_owner/placing_agent/adviser) — awaiting DT gap data; ② share-capital windowed design for heavy issuers (02318/00941 — confirmed systemic capacity limit); ③ fundamentals parser deep refinement (insurer labels, wrong-scale); ④ Monitor/Alerts (Phase 1 item 6, by design last); ⑤ streamlit version pin (home machine 26 test failures); ⑥ accumulation rotation completes over ~4-day cycles — partially-done slices resume next cycle (idempotent), not a defect.
