...
- [ ] **0.5** Scoped fixes from the 0.4 run, in order: **FIX-1** dynamic fundamentals fetcher (resolve a stock's annual/interim report PDFs from HKEXnews search at runtime → feed the existing PDF parsers; delete `HKEX_FUNDAMENTAL_DOCUMENTS` whitelist). **FIX-2** dynamic document-entity discovery feeding `DOCUMENT_ENTITY_SPECS` (same discovery layer). **FIX-3** announcements/timeline async-job pattern (reuse the DI job infra) or windowed chunking so 5-year pulls stop OOM-ing the free container; add a server-side chunked runner for DI full-window pulls. Then re-run this same 0.4 gate until 18/18 (middle gap stays DEFERRED by design). **Pack-verified answers for all three fixes are stored at `docs_reference_evidence/latest_reference_updates/2026-09-18_FRIEND_PACK_ANSWERS_FOR_FIX123.md`** (FIX-2 = full working reference in the pack; FIX-1 = discovery half proven, parser half ours to build; FIX-3 = friend caps at 2y, we chunk+async+accumulate).
...
1. **0.5 fixes** (see §7 0.5 + pack-answers digest `docs_reference_evidence/latest_reference_updates/2026-09-18_FRIEND_PACK_ANSWERS_FOR_FIX123.md`): FIX-1 dynamic fundamentals + FIX-2 dynamic document-entity discovery (kills both whitelist root causes), FIX-3 announcements/timeline/DI async+chunked pattern. Then re-run the 0.4 gate on a fresh unseen code — target 18/18 with the middle gap still DEFERRED.
2. DI is production-proven (00388/01810/02318 = 966 rows total in Turso); do not regress it while fixing the above.
3. Production DB now: disclosure_interests 00388=108 / 01810=230 / 02318=628; 02318 has announcements 236, holdings 357, stock row 1; raw_provenance 202.
4. Local tooling on the company machine (workspace root, outside repo): `friend_pack/` (extracted authoritative pack), `turso_query.py` (Turso HTTP helper), `gate_04.py` (gate runner), `debug_dion.py` (DION flow reproducer with dumps).
5. **Round-4 notes digested 2026-09-19** → `docs_reference_evidence/latest_reference_updates/2026-09-19_FRIEND_ROUND4_DIGEST_RTSS_TAPE_WARM_TURSO.md`. Key takeaways: (a) friend's warm cache is shallow (summaries, history-date unconfirmed) — our Turso/archive warm path already serves dated full history; (b) **Longbridge tape is today-only — any tape/爆量 history must be captured daily or it is lost forever (Phase-1 candidate, Evidence-Cache pattern); 爆量訊號已證實冇超額報酬（616 樣本 p=0.600）— future screener must be labelled 報時+累積, not buy signal**; (c) **RTSS 搜股系統 = separate future project** (own stack: longport+libsql+streamlit, private repo) — reads from our data layer, never built inside this platform (§2 governance); (d) **CCASS middle-gap data = Joe's Drive folder** (link in the 14092026 note) — build the labelled import path when it lands; DT's six 財技事件 types (配股/供股/全購/合股/拆股/CB) map 1:1 onto our Phase-1 event_type; (e) `15092026 原webbsite係點拎DATA.md` is an EMPTY file on the Drive — Joe to re-upload if it had content.
