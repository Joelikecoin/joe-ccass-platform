...
8d3. **ESCALATION 2026-09-25 (Friday, company machine) — Longbridge broker_holding_detail STILL empty on the 4th consecutive trading day** (single-stock probe 02020 → same ValueError, no rows; 09-22/23/24/25). Per the >3-day rule: **Joe action — ① open 長橋 app → CCASS/持股披露 data still displays? ② check open.longportapp.com developer announcements for CCASS API changes ③ if token entitlement lapsed, regenerate via device flow → new token to ZCode → Render env update via API.** Platform-side: fail-loud labelled, Turso holds data through 09-21 positions (5,987 rows), daily auto-retry continues, other domains unaffected. Note: a second remote branch `p0-runtime-api-key-fingerprint-proof` (plain name, at 66da477) appeared from home-session pushes — harmless duplicate pointer.
