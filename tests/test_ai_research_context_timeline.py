...
def test_ai_research_context_timeline_captures_snapshot_order_and_links(
    current_response, previous_response
):
    entry = _entry(current_response, previous_response)
    timeline = build_ai_research_context_timeline(entry)

    assert timeline.available is True
    assert timeline.current_snapshot_reference is not None
    assert timeline.current_snapshot_reference.snapshot_id == 101
    assert timeline.previous_snapshot_reference is not None
    assert timeline.previous_snapshot_reference.snapshot_id == 100
    assert len(timeline.historical_snapshot_metadata) == 4
    assert timeline.historical_snapshot_metadata[0].snapshot_id == 98
    assert timeline.historical_snapshot_metadata[1].snapshot_id == 99
    assert timeline.historical_snapshot_metadata[2].snapshot_id == 100
    assert timeline.historical_snapshot_metadata[3].snapshot_id == 101
    assert timeline.timeline_items[0].order == 1
    assert timeline.timeline_items[-1].order == 4
    assert timeline.timeline_items[-1].role == "current"
    assert timeline.timeline_items[-1].comparison_reference
    assert timeline.timeline_items[-1].change_summary_reference
    assert timeline.linked_comparison_references
    assert timeline.linked_change_summary_references
    assert "AI research context timeline:" in timeline.summary
    assert "snapshot_ordering=" in timeline.summary


def test_ai_research_context_timeline_markdown_handles_unavailable_state():
    markdown = build_ai_research_context_timeline_markdown(None)
