...
from app.services.job_runner import JobStore, start_job, run_job, run_entity_job, run_event_job
...
async def _temporary_token_proof(x_cross_source_proof: str | None = Header(default=None)):
    ...
    try:
        ...
        repository.put_many(canonical); after=repository.records(); first=max(0, len(after)-len(before)); stages.append({"stage_name":"turso_write","status":"PASS","rows_seen":len(canonical),"rows_written":first})
        repository.put_many(canonical); final=repository.records(); second=max(0, len(final)-len(after)); stages.append({"stage_name":"second_run_idempotency","status":"PASS","rows_seen":len(final),"rows_written":second})
...
async def _job_resume(job_id: str, x_api_key: str | None = Header(default=None)):
    ...
    if job["status"] in {"FAILED","BLOCKED","QUEUED"}:
        runner = (
            run_entity_job if job["job_type"] == "ENTITY_RESOLUTION_PRODUCTION_ACCEPTANCE"
            else run_event_job if job["job_type"] == "EVENT_PRODUCTION_ACCEPTANCE"
            else run_job
        )
        asyncio.create_task(runner(_job_store, job_id))
