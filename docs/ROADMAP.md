...
| Latest Holdings | P1-01至P1-07已形成完整vertical slice：approved Webb-site／Google CSV fetch、純parser、normalized persistence、collector、persistent LKG／freshness、public `participant_name`／`pct_of_ccass`／`issued_shares_as_of`、full-snapshot product invariants、diagnostics、limit-safe summary及canonical／legacy API與OpenAPI離線驗收。 |
...
| Collector | 經 `CcassService` 重用 `auto|webbsite|google_drive_csv` routing；支援dry-run、complete/partial honesty、normalized idempotent save、batch/per-stock run/error accounting及atomic CSV；P1-06把stale LKG記作`PARTIAL`，P1-07令source-neutral product validation的PARTIAL同樣不可promote並完整輸出diagnostics。 | 尚無 history export、完整 retry policy exposure或合法 live/golden batch 驗收；historical backfill 已由獨立 P1-03 CLI 完成。 |
...
| Reports and Copy | 固定九節 Markdown、error report、Copy Report、Copy for ChatGPT safety header及安全UTF-8 clipboard encoding；Holdings report已列issued-shares-as-of及每行pct-of-CCASS。 | 報告未涵蓋完整 sections；Copy 首段未以結構化方式固定列出實際 source/date/T+2/warnings；無 bilingual/localized export。 |
...
| FastAPI | `/health`、canonical `/api/v1/stocks/{stock_code}/holdings`、legacy holdings JSON及Markdown report；query/Bearer/X-API-Key auth；共用Holdings service；additive欄位及OpenAPI有離線測試。 | Project Specification 的 stock/holdings/changes/big-changes/concentration/rainbow/announcements/prices/report/source-status versioned routes 大多未有；partial/error envelope 及 query-key log redaction 未完整驗證。 |
...
| Tests | 137個離線tests；P1-07新增full-snapshot product invariants、denominator/date mismatch、public fields、limit-safe API/OpenAPI、LKG round-trip及collector diagnostics tests，並保留P1-01至P1-06全部regression。 | Project Specification矩陣仍缺rainbow/i18n/完整API/MCP/exports/golden live/visual tests。 |
...
- Done：6；Partial：19；Not Started：8；總計33個功能單位；Remaining Gaps共27個。
...
- Phase 1仍未通過：Latest Holdings已完成第一個核心vertical slice；合法active source多日真實snapshots／golden evidence及Changes、Big Changes、Concentration vertical slices仍欠缺，Phase 2不得提前開始。
- P1-05及P1-06已由CTO批准；`P1-07 — Complete Latest Holdings Vertical Slice`已完成實作並等待CTO Review。本輪只記錄實際狀態，不選取或開始下一個TASK。
...
| 2 | Source routing, fallback and cache | Partial | `P1-06`已完成persistent LKG、freshness/stale age及last source error；仍欠cross-source conflict policy及Phase 4公開diagnostics，只可使用registry批准來源。 |
| 3 | Historical Snapshot | Partial | 由合法active flow保存並驗證多日真實snapshots，補正式history query／delivery及golden evidence；不得用latest、插值或synthetic冒充。 |
| 4 | Changes | Partial | 只比較已驗證完整snapshot pair，補relative delta、日期／source／completeness metadata、service及tests。 |
| 5 | Big Changes | Partial | 在Changes pair contract上把threshold/basis/version集中config，補percentage/relative basis及完整snapshot保證。 |
| 6 | Concentration | Partial | 依完整snapshot補denominator freshness、partial rules、outside CCASS及historical timeline；不得以截斷holdings計算。 |
| 7 | Possible Transfer Patterns | Partial | 依已驗證Changes補可追溯tolerance/version、dates/source metadata、pair audit及固定免責。 |
| 8 | Collector | Partial | 核心idempotency/run accounting及stale LKG accounting已完成；待history export、完整retry policy exposure及合法live/golden batch evidence。 |
| 9 | Error and partial-success contract | Partial | 隨Phase 1 services補`DATA_STALE`／`PARTIAL_DATA`／`INVALID_SCHEMA`、safe details、partial sections及stale語義，不先建立空envelope。 |
| 10 | Tests | Partial | 隨上述slice補純parser/content guards、完整snapshot pair、四sections、service/API及合法多日/golden evidence。 |
| 11 | Security and logging | Partial | 補自動secrets scan、API access-log query redaction、auth failure矩陣及raw/report sensitive-field policy tests。 |
| 12 | HKEX SDW and manual official import | Not Started | Phase 1 golden抽樣前先取得明確source audit／批准；只可低頻官方或manual import，不得未核准自動化。 |
| 13 | Rainbow Data Engine | Not Started | 依賴Phase 1可信多日完整snapshots；完成matrix、Top N/Others、partial missing及stable colours。 |
| 14 | Price History | Not Started | Phase 2才audit adapter並完成model/store、adjustment/missing warnings、API/UI/export。 |
| 15 | Streamlit UX | Partial | Phase 2對齊navigation、tables、rainbow/concentration/price controls及desktop/mobile evidence；不得以UI先行冒充資料完成。 |
| 16 | i18n | Not Started | Phase 2建立`zh_HK`／English registry、fallback warning及不重新fetch的language state。 |
| 17 | HKEX Announcements | Not Started | Phase 3官方index/PDF limits、languages、event tags、failure isolation及delivery surfaces。 |
| 18 | Company | Partial | Phase 3補正式section、identifiers、name history、日期及source merge。 |
| 19 | Raw Previews and full Excel/JSON exports | Not Started | Phase 3完成Raw Preview、Excel All Sections、Raw Tables JSON及各section structured exports。 |
| 20 | Downloads | Partial | 依已完成sections補CSV/JSON/Excel、preview、metadata/warnings、encoding及安全檔名tests。 |
| 21 | Reports and Copy | Partial | 依完整sections固定實際source/date/T+2/warnings首段及localized exports。 |
| 22 | Source registry and diagnostics | Partial | Phase 4補runtime last success/failure、freshness/latency/cache telemetry及source-status service；P1-04 internal registry已完成，不重做。 |
| 23 | FastAPI | Partial | Phase 4依真實services完成versioned endpoints、stable envelope、auth/redaction及OpenAPI tests。 |
| 24 | MCP | Partial | Phase 4完成八個thin tools，共用service並補error/schema/deployment tests。 |
| 25 | Supplemental source audits | Not Started | 核心來源及sections穩定後才audit；HKEX DI/SDI、同花順、price fallbacks、AAStocks維持disabled/unverified。 |
| 26 | Deployment and operations | Partial | Phase 5補source metrics、cold-start/recovery、log rotation/remove scripts及scheduler文件；不自行安裝。 |
| 27 | Golden and public acceptance | Not Started | 最後執行合法live數字核對及公開Streamlit/API/MCP/mobile/language/download驗收；不得以offline fixture代替。 |
...
|---|---|---|---|
| 合法active CCASS source可解析identity並保存多日真實snapshots | ❌ 未完成 | Webb-site可由code核對identity但只提供latest；Google CSV exact-date只有批准import flow及offline fixtures，未有production多日／golden evidence。 | P1-05 guarded parser、P1-06 persistent LKG及P1-07完整Holdings contract已完成；下一依賴是合法多日資料。多日資料只能由日常collector累積或另行獲批的真實import，不能造數。 |
...
| Holdings／Changes／Big Changes／Concentration有真實資料、stable metadata/errors及API/service tests | ❌ 未完成 | Holdings已有full-snapshot validation、stable metadata/errors及canonical/legacy API tests；其餘三個section仍主要對supplied responses作compute，沒有完整snapshot pair service或目標API。 | Latest Holdings slice已完成；legal complete snapshot pairs → Changes/Big Changes/Concentration services → Phase 1 API/service tests。 |
...
| Ruff／Pytest／smoke／secrets及部署可用性 | ✅ 目前通過 | P1-07基準為137個offline tests；Ruff及Full Pytest通過，最終diff、Markdown、UTF-8、安全、database、Scope及public contract scans列入提交gate。 | 不等於公開部署或live source驗收；公開acceptance維持Phase 5。 |
...
Phase 1尚未完成條件：多日真實snapshots、Changes／Big Changes／Concentration及其service/API tests、合法golden抽樣。

依賴次序：Latest Holdings vertical slice已完成 → legal multi-day snapshots → Changes／Big Changes／Concentration → Phase 1 service/API tests → 獲批golden cross-check。任何HKEX SDW automation、source legality、credential或公開breaking schema需要CTO另行批准。
